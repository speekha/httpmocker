...
/**
 * Mapper object allowing to load and save mock scenarios. It is basically a file parser,
 * allowing to convert lists of Matcher objects to text file (JSON or other) and back
 */
interface Mapper {
 ...
 */
fun Mapper.readMatches(stream: StreamReader): List<Matcher>? = deserialize(stream.readAsString())
 ...
 * @return the corresponding data objects
 */
fun Mapper.readMatches(file: FileAccessor): List<Matcher>? = readMatches(file.getReader())
 ...
 * @param matchers the list of Matchers to serialize
 */
fun Mapper.writeValue(outputStream: StreamWriter, matchers: List<Matcher>) = outputStream.use {
    it.write(serialize(matchers).encodeToByteArray())
}
